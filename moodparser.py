...
def albumsentiment(artistname,albumname):
    ...
    cursor=conn.cursor()
    cursor.execute("INSERT INTO albumsentiment(albumname, artistname, sentiment) VALUES (%s, %s, %s)", (albumname,artistname,avgsentiment))
    conn.commit()
    return [albumname,artistname,avgsentiment]
            
def albumsentimentselect(artistname,albumname):
    query= "SELECT artistname, albumname, sentiment FROM albumsentiment WHERE artistname='{0}' AND albumname='{1}'".format(artistname,albumname)
    cursor=conn.cursor()
    cursor.execute(query)
    data=cursor.fetchone()
    if data!=None:
